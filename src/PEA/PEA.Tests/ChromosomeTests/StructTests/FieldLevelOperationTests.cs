...
namespace Pea.Tests.ChromosomeTests.StructTests
{
	...
	public class FieldLevelOperationTests
	{
		...
		public void DoubleFieldOperator_Set_ShouldSet(double change, double expected)
		{
			...
			var parameterSet = new ParameterSet(new List<PeaSettingsNamedValue>() { new PeaSettingsNamedValue(Chromosome.ParameterNames.MutationIntensity, change) }, ParameterSource.UserSetting);
